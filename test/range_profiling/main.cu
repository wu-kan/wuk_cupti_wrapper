...
inline void
CuptiProfilerHost::SetUp(std::string chipName,
                         std::vector<uint8_t> &counterAvailibilityImage) {
  m_chipName = chipName;
...
inline void CuptiProfilerHost::PrintProfilerRanges() {
  ...
  for (size_t rangeIndex = 0; rangeIndex < m_profilerRanges.size();
       ++rangeIndex) {
...
inline RangeProfilerTarget::RangeProfilerTarget(CUcontext ctx,
...
inline CUptiResult RangeProfilerTarget::GetChipName(size_t deviceIndex,
                                                    std::string &chipName) {
...
struct ParsedArgs {
  ...
  std::vector<const char *> metrics = {"sm__ctas_launched.sum"};
};
